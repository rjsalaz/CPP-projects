...
void quicksort3(int low, int high, vector<int>& arrayToBeSorted);
void partition(int low, int high, int& piviotPoint, vector<int>& arrayToBeSortedd);
...
int main()
{
	...
	cout << "Quick Sort 3 algorithm with Unsorted Input" << endl;
...
void test(int testIndex, vector<int>& arrayToBeSorted)
{
	...
	quicksort3(0, arrayToBeSorted.size() - 1, arrayToBeSorted); // call quick sort 
...
void quicksort3(int low, int high, vector<int>& arrayToBeSorted)
{
	...
	if (high - low > 16)
	{
		int random = (rand() % ((1 + high - low)) + low);
		swap(arrayToBeSorted[low], arrayToBeSorted[random]);
	}

	if (high > low)
	{
		partition(low, high, piviotPoint, (arrayToBeSorted));
		quicksort3(low, piviotPoint - 1, arrayToBeSorted);
		quicksort3(piviotPoint + 1, high, arrayToBeSorted);
	}
}
